...
#include <fstream>
#include <string>		// fájlból olvasunk, írunk majd
...
public:
   ...
   */
    ...
    {
        melyseg = 0;
        kiir_po (&gyoker, std::cout);
    }

    void kiir_pr (void)
    {
        melyseg = 0;
        kiir_pr (&gyoker, std::cout);
    }
  ...
  */
    ...
    void kiir_po (std::ostream & os)
    {
        melyseg = 0;
        kiir_po (&gyoker, os);
    }

    void kiir_pr (std::ostream & os)
    {
        melyseg = 0;
        kiir_pr (&gyoker, os);
    }
...
private:
    ...
    void kiir_in (Csomopont * elem, std::ostream & os)
    {
        if(elem != NULL)
         {
            ++melyseg;
            kiir_in (elem->nullasGyermek (), os); //bal
                        // 1-el nagyobb mélység, ezért -1
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;//gyöker
            kiir_in (elem->egyesGyermek (), os); //jobb

            --melyseg;
        } 
    }
    void kiir_po (Csomopont * elem, std::ostream & os)
    {
        if(elem != NULL)
        {
            ++melyseg;
            kiir_po (elem->nullasGyermek (), os); //bal
                        // 1-el nagyobb mélység, ezért -1
            kiir_po (elem->egyesGyermek (), os); //jobb
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;//gyöker        
            --melyseg;
    ...
    void kiir_pr (Csomopont * elem, std::ostream & os)
    {
            if(elem != NULL)
            {
            ++melyseg;
            ...
            for (int i = 0; i < melyseg; ++i)
                os << "---";
            os << elem->getBetu () << "(" << melyseg - 1 << ")" << std::endl;//gyöker
            kiir_pr (elem->nullasGyermek (), os);
            kiir_pr (elem->egyesGyermek (), os); //jobb
            --melyseg;
            }
    }
...
main (int argc, char *argv[])
{
    ...
    {
        binFa.kiir_po(kiFile);
    }
    else if(a == "-pr")
    {
        binFa.kiir_pr(kiFile);
    }
    //cout << binFa.kiir (); // így rajzolt ki a fát a korábbi verziókban de, hogy izgalmasabb legyen
    // a példa, azaz ki lehessen tolni az LZWBinFa-t kimeneti csatornára:
   ...
   // kiFile << "Nullások száma= " << binFa.nullasok << std::endl;
   // kiFile << "Egyesek száma= " << binFa.egyesek << std::endl;
...
kiíratás: while ciklus után
if (*((*++argv) + 1) == 's') 
    //standard outpura a kiiratás
    {
