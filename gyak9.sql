...
--------------------------------------10 gyak.---------------------------------------------------------------------------------
delete from tulaj
    where mod(azon , 2) = 1;----azokat a sorokat töröljük, ahol az azonosítú páratlan szám (2-vel osztva 1 maradékot ad)
...
--      vagy
--update <táblanév>
--  set (<oszlop>, ...) = (select ....ez mindig egysoros lekérdezés lesz)
--  [where <feltétel>];


update tulaj set szul_dat = sysdate;

--nagybetûssé alakítani azokat a neveket, ahol a szem. szám kevesebb mint 11 karakter, valamint alakítsuk 11 karakterré a szmélyi számot, úgy, hogy a végére íronk megfelelõ mennyiségû 0-t

update tulaj set nev = upper(nev),
                szemelyiszam = substr(trim(szemelyiszam) || '00000000000' , 1, 11) --hozzáadunk 11 nullát és levágjuk az elsõ 11-et
            where length(trim(szemelyiszam)) < 11; --trim az elejérõl és a végérõl is leszedi a szóközöket
            
            --(trim(trailing' ') a végérõl leszedi a szóközt
            --(trim(leading)'.') az elejérõl leszedi a  . -ot

--töröljük ki a darabszámot és írjuk be újra, a tag táblából
...
update tag 
    set darab = 
            ...
            where t.vezeteknev = trim( trailing '+' from tag.vezeteknev) --leszedjük a +-okat a nevek végérõl
                and t.keresztnev = trim( trailing '+' from tag.keresztnev)
                    and to_char(t.szuletesi_datum, 'ds') = tag.szuldat);--konvertáltuk a dátumot karakterré, hogy a két táblában megegyezzen a formátum
                    
                    
--DCL Data Control Language
-- tanzakciókezelés (nem lesz a ZH-ban) tranzakciókezelõ utasítások : -commit (F11 hotkey) (véglegesítés, hatása már nem vonható vissza), 
--                                                                     -rollback (F12 hotkey) (visszavonja az utasításokat a tranzakció kezdetéig)
--                                                                     -set autocommit on; (minden utasítás után végrehajtódik, ezután a rollback nem csinál semmit)
--                                                                     -savepoint <név>; a megadott néven mentési pontot hozunk létre, ekkor a rollback <név>; csak a mentési pontig töröl vissza,
--
-- jogosultságkezelés : grant (jogot oszt)
--                      revoke (jogot von vissza)
--
--  grant [<jogosultság>, [...] | all [privileges]]
--          [on <sémaobjektum>]
--          to [<felhasználó>, [...] | public]
--          [with grant option];--ez által továbbadhatják a jogokat a felhatalmazottak, jogláncolat alakul ki
--
--  jogosultság lehet: -rendszerjog (nem lesz a ZH-ban)
--                     -objektumjogosultság:    select 
--                                              insert [(<oszlopnév>, ...)] 
--                                              delete 
--                                              update [(<oszlopnév>, ...)]
--                                              references [(<oszlopnév>, ...)] (az illetõ felhaszáló hivatkozhat erre a táblára külsõ kulccsal)
--
-- revoke [<jogosultság>, [...] | all [privileges]]
--          [on <sémaobjektum>]
--          from [<felhasználó>, [...] | public];
--  
--
--                     -objektumjogosultság:    select 
--                                              insert [(<oszlopnév>, ...)] 
...
--<kifejezés><hasonlító operátor  =, <, >, <=, >= > all (select...)   (bármely v1, v2, v3,.... vn)
--                                                  any (select)      (létezik v1, v2, v3,.... vn)

--   '= any'   ekvivalens az 'in' operátorral
--  '<> all'   ekvivalens a 'not in' operátorral

--írjuk ki azon tagok adatait, akik az összes nyugdíjasnál fiatalabbak
...
--mennyit kerestek összesen azok a szerzõk, akiknek a könyveit csak nyugdíjasok vettek ki (5 táblát kell összekapcsolni)

--hogy hívják azokat a szerzõket, akiknek a könyveit nyugdíjasok IS kikölcsönözték